...
#same as controller, we just have pdot, qdot, rdot = 0.
...
class controller(object):
    def __init__(self):
        ...
        #solution vector: n_x, n_y, n_z, epsilon, p, q, r, omega1, omega2, omega3, omega4
        self.initial_guess = [0,0,1, 0.001, 0.2,6, 19,  450,560,450,  0]
        #self.initial_guess = [0,0.289,0.958,0, 5.69, 18.89, 0.1,437.00751841778504,562.2395063000737,437, 0]
        
    def equations(self, vars): 
        n_x, n_y, n_z,p, q, r, epsilon, omega1, omega2, omega3, omega4 = vars[0], vars[1], vars[2], vars[3], vars[4], vars[5], vars[6], vars[7], vars[8], vars[9], vars[10]
        #equations
        eq1 = self.k_f*(omega2**2-omega4**2)*self.l - (self.I_T_zz-self.I_T_xx)*q*r-self.I_P_zz*q*(omega1+omega2+omega3+omega4)
        eq2 = self.k_f*(omega3**2-omega2**2)*self.l+(self.I_T_zz-self.I_T_xx)*p*r+self.I_P_zz*p*(omega1+omega2+omega3+omega4)
        eq3 = -self.gamma*r+self.k_tau*self.k_f*(omega1**2-omega2**2+omega3**2-omega4**2)
        eq4 = epsilon*p-n_x
        eq5 = epsilon*q-n_x
        eq6 = epsilon*r-n_x
